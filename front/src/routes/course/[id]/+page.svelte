        ...
        <main class="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
          ...
          {#if rq.member.id === course.writer_id || rq.isAdmin()}
            <div class="mx-2 items-center">
              <div class="mb-2">
                <a
                  href="/course/{$page.params.id}/edit"
              ...
              <div>
                {#if !courseConfirm}
                  <button
                    ...
                    class="h-12 font-semibold inline-block px-4 py-2 border border-gray-400 text-gray-800 bg-white hover:bg-gray-700 hover:text-white rounded-md shadow-sm text-sm font-medium focus:outline-none"
                    >강좌 공개</button
                  >
                {:else}
                  <button
